...
def analyze_movement(file_path: str, start_time: datetime, category: str, sample_name: str) -> pd.DataFrame:
    ...
    try:
        ...
        # 経過時間を計算（データ取得開始からの時間）
        elapsed_hours = [(t - start_sec) / 3600 for t in times]
        ...
        # 時間ごとの平均を計算
        hourly_means = results.groupby(results['hour'].astype(int))['vector_magnitude'].mean().reset_index()
...
def plot_movement_by_category(all_results: pd.DataFrame, output_dir: str):
    ...
    for category in categories:
        ...
        for sample in category_data['sample_name'].unique():
            ...
            plt.plot(data['hour'], data['vector_magnitude'],
    ...
    plt.xlabel('Hours after start (Hour 1-25)')
    ...
    plt.xlim(0, 24)  # x軸の範囲を0-24時間に設定
    
    # y軸を0から開始
    plt.ylim(bottom=0)
    
    # グラフを保存（凡例が切れないように余白を調整）
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'movement_analysis_by_category_from_starttime.png'), 
                bbox_inches='tight', dpi=300)
    plt.close()

def process_filtered_data(metadata_file: str, output_dir: str):
    """メタデータに基づいてフィルタ済みの座標データを処理"""
    # メタデータの読み込み（sample_nameを文字列として読み込む）
    metadata = pd.read_csv(metadata_file, dtype={'sample_name': str})
    all_results = []
    
    for _, row in metadata.iterrows():
        # メタデータで指定されたファイル名を直接使用
    ...
    if all_results:
        ...
        # 結果をCSVとして保存
        combined_results.to_csv(os.path.join(output_dir, 'hourly_movement_data_from_starttime.csv'), index=False)
...
def main():
    ...
    if results is not None:
        ...
        print(f"- Graph: {os.path.join(args.output, 'movement_analysis_by_category_from_starttime.png')}")
        print(f"- Data: {os.path.join(args.output, 'hourly_movement_data_from_starttime.csv')}")
